...
export async function convertStatisticsFilesInput(input) {
    if (!input || input.length === 0)
        throw new Error("No file input provided.");
    if (!(input instanceof FileList))
        throw new Error("Input is not a FileList.");
    const csvTextArray = [];
    Array.from(input).forEach((file) => {
        if (file.name.toLowerCase().endsWith(".csv")) {
            ...
            throw new InvalidFileTypeError(`Invalid file type: ${file.name}. Expected a CSV file.`);
        }
    });
    return csvTextArray;
}
export async function convertNamesFileInput(input) {
    if (!input || input.length === 0) {
        throw new Error("No file input provided.");
    }
    const file = input[0];
    ...
    if (!["text/csv", "text/plain"].includes(fileType)) {
        throw new InvalidFileTypeError(`Invalid file type: ${file.name}. Expected a CSV or TXT file.`);
    }
    let names = [];
    file
        .text()
...
export function convertNamesTextInput(input) {
    if (!input || input.trim() === "") {
...
export function validatePasskeyInput(input) {
    if (!input || input.trim() === "") {
...
export function convertForgivenessDegreeInput(input, defaultValue = 0) {
    if (!input) {
        return defaultValue;
    }
...
export function convertThresholdInput(input, defaultValue = 0.8) {
    if (!input) {
        return defaultValue;
    }
